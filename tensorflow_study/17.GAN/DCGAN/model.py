...
class DCGAN(object):
    def __init__(self, sess, input_height=108, input_width=108, crop=True,
                 ...
                 y_dim=None, z_dim=100, gen_first_dim=64, dis_first_dim=64,
        ...
        self.gf_dim = gen_first_dim
        self.df_dim = dis_first_dim
    ...
    def build_model(self):
        ...
        self.z_sum = histogram_summary("z", self.z)  # 噪声向量

        # generator生成图，y类似分类label,如果是mnist问题，则需要0-9表示生成那种数字
        self.G = self.generator(self.z, self.y)
        self.D, self.D_logits = self.discriminator(
            inputs, self.y, reuse=False)  # 输入样本过discriminator，获得分数
        self.sampler = self.sampler(self.z, self.y)  # 取样,不训练直接生成图片
        # 生成的G过discriminator，获得分数  通过 scope.reuse_variables() 共享discriminator的参数
        self.D_, self.D_logits_ = self.discriminator(
            self.G, self.y, reuse=True)

        self.d_sum = histogram_summary("d", self.D)  # 样本的分数，未经过sigmoid
        self.d__sum = histogram_summary("d_", self.D_)  # G生成的图片的分数，未经过sigmoid
        self.G_sum = image_summary("G", self.G)  # 生成的图片
        ...
        self.d_loss_real = tf.reduce_mean(
            sigmoid_cross_entropy_with_logits(self.D_logits, tf.ones_like(self.D)))  # 样品的评分越接近1越好（对于D）
        self.d_loss_fake = tf.reduce_mean(
            sigmoid_cross_entropy_with_logits(self.D_logits_, tf.zeros_like(self.D_)))  # G生成的图片的评分越接近0越好（对于D）
        self.g_loss = tf.reduce_mean(
            sigmoid_cross_entropy_with_logits(self.D_logits_, tf.ones_like(self.D_)))  # G生成的图片越真越好（对于G）

        self.d_loss_real_sum = scalar_summary("d_loss_real", self.d_loss_real)
        self.d_loss_fake_sum = scalar_summary("d_loss_fake", self.d_loss_fake)

        self.d_loss = self.d_loss_real + self.d_loss_fake  # D的总Loss

        self.g_loss_sum = scalar_summary("g_loss", self.g_loss)  # G的Loss
        self.d_loss_sum = scalar_summary("d_loss", self.d_loss)  # D的Loss
    ...
    def train(self, config):
        d_optim = tf.train.AdamOptimizer(config.learning_rate, beta1=config.beta1) \
            .minimize(self.d_loss, var_list=self.d_vars)  # 先固定G的参数，训练一个D的参数
        g_optim = tf.train.AdamOptimizer(config.learning_rate, beta1=config.beta1) \
            .minimize(self.g_loss, var_list=self.g_vars)  # 固定D的参数，训练一个G的参数
        try:
        ...
        self.g_sum = merge_summary([self.z_sum, self.d__sum,
                                    self.G_sum, self.d_loss_fake_sum, self.g_loss_sum])  # 统计噪声向量z，G图片分数，G生成的图片，G生成图片的的Loss， G的Loss
        self.d_sum = merge_summary(
            [self.z_sum, self.d_sum, self.d_loss_real_sum, self.d_loss_sum])  # 统计噪声向量z，真实样品图片分数，真实图片的的Loss， D的Loss
        ...
        # 读取checkpoint, 返回读取结果bool和checkpoint的训练的次数
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, int(batch_idxs)):
                ...
                counter += 1
                print("\rEpoch: [%2d/%2d] [%4d/%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f"
                ...
                if np.mod(counter, 500) == 2:  # 每训练一个批次记1次，训练500次存model
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            if not self.y_dim:
                ...
                h4 = linear(tf.reshape(
                    h3, [self.batch_size, -1]), 1, 'd_h4_lin')  # nn层
